...
def get_exercises_from_db():
    exercises = []
    with open("./src/webapp/homeworkouts_org_exercises.json") as file:
        for line in file:
        ...
        while exercise_sum != (3 * self.user_requirements["level"]):
            ...
            for i in exercise_list:
                if exercise_sum == 3 * self.user_requirements["level"]:
                    break
                if i["level"] > self.user_requirements["level"] or i["level"] > (
                    (3 * self.user_requirements["level"]) - exercise_sum
                ):
                    continue
                if needed_groups == []:
