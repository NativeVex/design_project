...
class MealplanGenerator(data_src.DataStructures):
    ...
    def _calculate_meal_plan_nutrition(self, recipes):
        ...
        for i in recipes:
            nutrition_data = self._sum_nutritional_values(nutrition_data, i["nutritional value"])
    ...
    def _meal_plan_RSS(self, health_requirements, meal_plan):
        ...
        RSS = 0
        for i in meal_plan:
            offset = self._diff_nutritional_values(health_requirements, i["nutritional value"])
            for j in offset:
                RSS += offset[j]**2
        return RSS
...
#this is the "head" of the code
    def gen_meal_plan(self) -> DataStructures.meal_plan:
        ...
        possible_meal_plans_iterator = itertools.combinations(self.recipes, meals_per_meal_plan)

        #print all possible meal plans
        lowest_RSS = 1000000000000
        for i in possible_meal_plans_iterator:
            current_meal_plan_RSS = self._meal_plan_RSS(self.user_health_requirements, i)
